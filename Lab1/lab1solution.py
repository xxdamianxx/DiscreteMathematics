...
# The example formula above is equivalent to (-True 'OR' (True 'AND' False)) in infix notation def evaluate_r(formula):
    # Provide solution here
def evaluate_r(formula):
	if type(formula[1]) is type(True):
		prop1 = formula[1]
	else:
		prop1 = evaluate_r(formula[1])
	...
	if formula[0] is "NOT":
		...
		if type(formula[2]) is type(True):
			...
			prop2 = evaluate_r(formula[2])

	if formula[0] is 'AND':
		return AND(prop1, prop2)
	elif formula[0] is 'OR':
		return OR(prop1, prop2)
	elif formula[0] is 'IF':
		return IF(prop1, prop2)
	elif formula[0] is 'IFF':
		return IFF(prop1, prop2)
